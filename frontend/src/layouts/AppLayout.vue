<template>
  <div class="flex h-screen bg-background dark:bg-dark-background text-on-surface dark:text-dark-on-surface">
    ...
    >
      <div class="mb-8 flex items-center h-8 flex-shrink-0" :class="isSidebarExpanded ? 'justify-start' : 'justify-center'">
      ...
      <nav class="flex-grow flex flex-col overflow-y-auto pr-2">
        <RouterLink v-for="item in navItems" :key="item.name" :to="item.path"
          ...
          :class="{ 
            'bg-primary/10 font-semibold text-primary dark:text-dark-primary': $route.name === item.name || $route.meta.parent === item.name,
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        <header class="p-4 border-b border-surface dark:border-dark-surface/50 flex justify-between items-center">
            <h2 class="text-xl font-medium">{{ $route.name }}</h2>
            <div class="flex items-center gap-2">
                <button @click="uiStore.zoomOut" class="p-2 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full">
                    <ZoomOut class="w-5 h-5"/>
                </button>
                <span class="font-semibold w-10 text-center">{{ uiStore.zoomLevel }}%</span>
                <button @click="uiStore.zoomIn" class="p-2 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full">
...
const allNavItems = shallowRef([
  ...
  { name: 'Purchase Order', path: '/purchase-orders', icon: ClipboardList, admin: true },
  ...
  { name: 'Riwayat Transaksi', path: '/history', icon: HistoryIcon, admin: true },
