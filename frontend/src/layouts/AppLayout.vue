<template>
  <div class="flex h-screen bg-background dark:bg-dark-background text-on-surface dark:text-dark-on-surface">
    ...
    <aside
      ...
      :class="isSidebarExpanded ? 'w-64' : 'w-20'" @mouseenter="isSidebarExpanded = true"
      ...
      <div class="mb-8 flex items-center h-8" :class="isSidebarExpanded ? 'justify-start' : 'justify-center'">
        ...
        <span class="text-2xl font-bold text-primary dark:text-dark-primary ml-2"
          :class="{ 'hidden': !isSidebarExpanded }">djlabware</span>
      </div>
      ...
      <nav class="flex-grow flex flex-col overflow-y-auto pr-2">
        <RouterLink v-for="item in navItems" :key="item.name" :to="item.path"
          ...
          <span class="ml-3 whitespace-nowrap" :class="{ 'hidden': !isSidebarExpanded }">{{ item.name }}</span>
        </RouterLink>
      </nav>
      ...
      <div class="mt-auto w-full">
        <div class="p-2 flex items-center" :class="isSidebarExpanded ? 'flex-row' : 'flex-col'">
          <div
            class="bg-primary text-white rounded-full w-10 h-10 flex items-center justify-center font-bold text-lg mb-2 sm:mb-0 flex-shrink-0">
            {{ userInitials }}
          </div>
        ...
        <button @click="authStore.logout()"
          class="w-full flex items-center px-4 py-3 mt-2 rounded-full hover:bg-red-500/10 text-red-500"
...
const allNavItems = shallowRef([
  ...
  { name: 'Daftar Purchase Order', path: '/purchase-orders', icon: ClipboardList, admin: true },
