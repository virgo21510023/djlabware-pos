<template>
  <div class="bg-gray-200 min-h-screen p-8">
    <div class="max-w-4xl mx-auto mb-4 flex justify-end gap-2 print:hidden">
        <button @click="downloadPDF" :disabled="isGeneratingPDF" class="bg-blue-600 text-white px-4 py-2 rounded shadow">
    ...
    <div v-else-if="deliveryOrder" id="printable-do" class="bg-white p-8 max-w-4xl mx-auto shadow-lg text-black flex flex-col min-h-[27cm]">
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
            ...
            <div class="text-right">
                ...
                <p class="text-gray-600"><strong>Tanggal:</strong> {{ formatTanggal(deliveryOrder.delivery_date) }}</p>
                <p class="text-gray-600"><strong>Ref. Invoice:</strong> {{ deliveryOrder.Invoice.invoice_number }}</p>
            </div>
        </div>
            ...
            <table class="w-full mb-6 text-sm">
                ...
                <tbody>
                    <tr v-for="(item, index) in deliveryOrder.items" :key="item.id" class="border-b">
                        ...
                        <td class="p-2 text-right align-top">{{ item.quantity_shipped }} {{ item.Product?.satuan }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
